...
É importante frisar que a quantidade de movimento (também chamada de <b>momento</b>) é uma grandeza abstrata, assim como energia, por isso, pode ser mais difícil de visualizá-la do que uma força, por exemplo.

Podemos calcular o <b>momento</b> linear de um objeto por meio da fórmula 
...
Essa análise é importante. Perceba a unidade do momento, e a unidade da <b>força</b>. Se dividirmos a unidade do momento por tempo novamente, teremos um $\text N$! 
...
Que é a <b>escrita original</b> da definição de força, feita por Newton em seu livro, <i>Principa Mathematica</i>!

A magnitude de uma força é calculada pela <b>variação do momento no tempo</b>. 

# Impulso

O <b>impulso</b> é uma grandeza vetorial que mede a variação do momento de algum objeto, relacionando nesse processo a força aplicada, o tempo percorrido e a quantidade de movimento. 
...
Nos utilizando da análise dimensional, podemos ver que o impulso é medido em <b>Newton-segundos</b>: um impulso com um módulo de 1 Newton-segundo equivale a uma força de 1 newton aplicada durante 1 segundo.
...
$$
F=\dfrac{\Delta Q}{\Delta t} \iff F \Delta t =\Delta Q \iff I = \Delta Q
$$

# Colisões

Colisões acontecem quando corpos se chocam, podendo ser de três tipos: <b>perfeitamente elásticas</b>, <b>parcialmente elásticas</b> ou <b>perfeitamente inelásticas</b>. 
...
<i>Exemplo de colisão elástica entre duas massas iguais. Animação sob CC-BY-SA, via <a href="https://commons.wikimedia.org/wiki/File:Elastischer_sto%C3%9F.gif" target="_blank">Wikimedia Commons</a>.</i>

![](https://upload.wikimedia.org/wikipedia/commons/e/e5/Elastischer_sto%C3%9F3.gif)

<i>Exemplo de colisão elástica entre duas massas desiguais. Animação sob CC-BY-SA, via <a href="https://commons.wikimedia.org/wiki/File:Elastischer_sto%C3%9F3.gif" target="_blank">Wikimedia Commons</a>.</i>
...
\begin{align*}
    ...
    \therefore m_1 u_1 - m_1 v_1 &= m_2 v_2 - m_2 u_2 \\
    \therefore m_1 (u_1 - v_1) &= m_2(v_2 - u_2)
\end{align*}
$$
...
\begin{align*}
    ...
    \therefore m_1 u_1^2 + m_2u_2^2 &= m_1v_1^2 + m_2v_2^2 \\
    \therefore m_1u_1^2 - m_1v_1^2 &= m_2v_2^2 - m_2u_2^2 \\
    \therefore m_1(u_1^2 - v_1^2) &= m_2(v_2^2 - u_2^2) \\
    \therefore m_1(u_1 + v_1)(u_1 - v_1) &= m_2(v_2 + u_2)(v_2 - u_2)
\end{align*}
$$
...
\begin{align*}
    ...
    \therefore u_1 + v_1 &= v_2 + u_2 \\
    \therefore u_1 - u_2 &= v_2 - v_1 \\
    \therefore -\Delta u &= \Delta v
\end{align*}
$$

Uma primeira informação interessante. A velocidade de aproximação pré-colisão é igual em módulo à velocidade de afastamento pós colisão entre os dois objetos.
...
\begin{align*}
    ...
    \therefore m_1(2u_1 - u_2 - v_2) &= m_2(v_2 - u_2) \\
    \therefore 2u_1m_1 - m_1u_2 - m_1v_2 &= m_2v_2 - m_2u_2 \\
    \therefore 2u_1m_1 &= m_2v_2 + m_1v_2 + m_1u_2 - m_2u_2 \\
    \therefore 2u_1m_1 &= v_2(m_1 + m_2) + u_2(m_1 - m_2) \\
    \therefore v_2(m_1 + m_2) &= 2u_1m_1 - u_2(m_1 - m_2) \\
    \therefore v_2 &= \dfrac{2u_1m_1 - u_2(m_1-m_2)}{m_1+m_2} \\
    \therefore v_2 &= \boxed{\dfrac{2m_1}{m_1+m_2}u_1 + \dfrac{m_2-m_1}{m_1+m_2}u_2}
\end{align*}
$$
...
\begin{align*}
    ...
    \therefore m_1(u_1 - v_1) &= m_2(u_1 + v_1 - 2u_2) \\
    \therefore m_1u_1 - m_1v_1 &= m_2u_1 + m_2v_1 - 2m_2u_2 \\
    \therefore -m_1v_1 - m_2v_1 &= -m_1u_1 + m_2u_1 - 2m_2u_2 \\
    \therefore -v_1(m_1 + m_2) &= u_1(m_2-m_1) - 2m_2u_2 \\
    \therefore v_1(m_1+m_2) &= 2m_2u_2 - u_1(m_2-m_1) \\
    \therefore v_1 &= \dfrac{2m_2u_2 - u_1(m_2-m_1)}{m_1+m_2} \\
    \therefore v_1 &= \boxed{\dfrac{m_1-m_2}{m_1+m_2}u_1 + \dfrac{2m_2}{m_1+m_2}u_2}
\end{align*}
$$
...
Nas colisões inelásticas, a energia cinética do sistema <b>não</b> é completamente conservada, sendo convertida em outras formas como <b>som e calor</b>, por exemplo. 

Quando uma colisão é <b>perfeitamente inelástica</b>, o máximo valor possível de energia cinética é perdido, “colando” os dois corpos num só, com ambos possuindo a mesma velocidade final.

![](https://upload.wikimedia.org/wikipedia/commons/d/de/Inelastischer_sto%C3%9F.gif)

<i>Colisão perfeitamente inelástica entre duas massas iguais. GIF sob CC-BY-SA, via <a href="https://commons.wikimedia.org/wiki/File:Inelastischer_sto%C3%9F.gif" target="_blank">Wikimedia Commons</a>.</i>
...
Para deduzir essa expressão, trabalhe com a <b>conservação do momento</b> (e o fato de que os corpos estão como um corpo só depois da colisão) e rearrange a expressão!
...
Definido como sendo a razão entre as velocidades após a colisão e antes da colisão, o <b>coeficiente de restituição</b> ($e$) é expresso pela equação abaixo: 

$$
e=\dfrac{|v_1-v_2|}{|u_1 - u_2|} = \dfrac{|\Delta v|}{|\Delta u|}
$$

Por meio do coeficiente de restituição, é possível montar uma <b>expressão geral</b> que abarca todos os cenários de colisões unidimensionais: 
...
Quando $e=1$, temos uma colisão <b>perfeitamente elástica</b>; caso $e=0$, temos uma colisão <b>perfeitamente inelástica</b>.